...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        configs.conf_thresh = 0.5
        configs.num_samples = None
        configs.nms_thresh = None    # no non-max suppresion for fpn resnet
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            output_decode = decode(outputs['hm_cen'], outputs['cen_offset'], outputs['direction'], outputs['z_coor'],
                                outputs['dim'], K=configs.K)
            output_decode = output_decode.cpu().numpy().astype(np.float32)
            output_post = post_processing(output_decode, configs)
            output_post = output_post[0]
            ...
            for j in range(configs.num_classes):
                if len(output_post[j]) > 0:
                    for det in output_post[j]:
                        ...
                        detections.append([1, _x, _y, 0.0, 1.50, _w, _l, -_yaw])  
